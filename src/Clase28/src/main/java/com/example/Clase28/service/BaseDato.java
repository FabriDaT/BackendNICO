...
public class BaseDato {
    ...
    public void crearTabla() {
        ...
        String TABLA_CREATE_TURNO = "DROP TABLE IF EXISTS TURNOS; " +
                "CREATE TABLE TURNOS" +
                "(ID BIGINT PRIMARY KEY AUTO_INCREMENT, PACIENTE BIGINT," +
                "ODONTOLOGO BIGINT,FECHA_TURNO VARCHAR(255));";
        try {
            ...
            statement.execute(TABLA_CREATE_TURNO);
            statement.close();
            connection.close();
    ...
    public String insert(String tabla){
        return switch (tabla) {
            case "PACIENTES" -> "INSERT INTO PACIENTES (NOMBRE, APELLIDO, DNI, FECHA_INGRESO, DIRECCION) VALUES(?,?,?,?,?);";
            ...
            case "TURNOS" -> "INSERT INTO TURNOS (PACIENTE, ODONTOLOGO, FECHA_TURNO) VALUES(?,?,?);";
    ...
    public String delete(String tabla){
        return switch (tabla) {
            ...
            case "TURNOS" -> "DELETE FROM TURNOS WHERE ID= ?;";
    ...
    public String select(String tabla){
        return switch (tabla) {
            ...
            case "TURNOS" -> "SELECT * FROM TURNOS WHERE ID = ?;";
            default -> "nombre de tabla o columna incorrecta";
        };
    }
    public String selectAll(String tabla){
        return switch (tabla) {
            ...
            case "TURNOS" -> "SELECT * FROM TURNOS;";
            default -> "nombre de tabla incorrecta";
        };
    }
    public String update(String tabla){
        return switch (tabla){
            ...
            case "TURNOS" -> "UPDATE TURNOS SET PACIENTE = ?, ODONTOLOGO = ?, FECHA_TURNO = ? WHERE ID = ?;";
            default -> "nombre de tabla incorrecta";
        };
    }
}
